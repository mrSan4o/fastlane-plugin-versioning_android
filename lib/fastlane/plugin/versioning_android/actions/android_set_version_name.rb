  ...
  module Actions
    ...
    class AndroidSetVersionNameAction < Action
      def self.run(params)
        ...
        new_version_name = Helper::VersioningAndroidHelper.get_new_version_name(gradle_file_path, version_name_key, params[:version_name], params[:bump_type])
        # bump_type ||= params[:bump_type]

        saved = Helper::VersioningAndroidHelper.save_key_to_gradle_file(gradle_file_path, version_name_key, new_version_name)

        if saved == -1
          UI.user_error!("Unable to set the Version Name(#{version_name_key}) in build.gradle file at #{gradle_file_path}.")
        end

        UI.success("☝️  Android Version Name(#{version_name_key}) has been set to: #{new_version_name}")
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(key: :version_name_key,
                                  env_name: "FL_ANDROID_SET_VERSION_NAME_VERSION_NAME_KEY",
