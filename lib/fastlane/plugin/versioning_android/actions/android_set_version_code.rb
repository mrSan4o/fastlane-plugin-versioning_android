  ...
  module Actions
    ...
    class AndroidSetVersionCodeAction < Action
      def self.run(params)
        ...
        new_version_code = Helper::VersioningAndroidHelper.get_new_version_code(gradle_file_path, version_code_key, params[:version_code])

        saved = Helper::VersioningAndroidHelper.save_key_to_gradle_file(gradle_file_path, version_code_key, new_version_code)

        if saved == -1
          UI.user_error!("Unable to set the #{version_code_key} in build.gradle file at #{gradle_file_path}.")
        end

        UI.success("☝️  Android Version Code(#{version_code_key}) has been set to: #{new_version_code}")
      ...
      def self.available_options
        [
          ...
          FastlaneCore::ConfigItem.new(key: :version_code_key,
                                  env_name: "FL_ANDROID_SET_VERSION_CODE_VERSION_CODE_KEY",
