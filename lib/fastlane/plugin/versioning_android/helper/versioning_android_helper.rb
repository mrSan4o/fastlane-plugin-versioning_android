module Fastlane
  module Helper
    class VersioningAndroidHelper
      ...
      def self.get_new_version_code(gradle_file, key, new_version_code)
        if new_version_code.nil?
          current_version_code = self.read_key_from_gradle_file(gradle_file, key)
      ...
      def self.get_new_version_name(gradle_file, key, new_version_name, bump_type = nil)
        if new_version_name.nil?
          new_version_name = self.read_key_from_gradle_file(gradle_file, key)
        end
